...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        return min(self.points[i].distance(other.points[j]) \
    ...
    def maxLinkageDist(self, other):
        ...
        self and the other point is from other. Uses the 
        Euclidean dist between 2 points, defined in Point."""
        return max(self.points[i].distance(other.points[j]) \
            for i in range(len(self.points)) \
            for j in range(len(other.points)))
    def averageLinkageDist(self, other):
        ...
        distance = 0.0
        count = 0
        for pair in set(itertools.product(self.points, other.points)):
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        self.add(Cluster(list(set(c1.members()).union(set(c2.members()))),c1.pointType))
    ...
    def findClosest(self, linkage):
        ...
        closest = None
        for cluster, other_cluster in itertools.combinations(self.members, 2):
            if closest == None:
                closest = (cluster, other_cluster, linkage(cluster, other_cluster))
            elif closest[2] > linkage(cluster, other_cluster):
                closest = (cluster, other_cluster, linkage(cluster, other_cluster))
...
def test():
    #points = buildCityPoints('C:\Users\Nate\Documents\edx-6.00.2x-problem-sets\problem_set_6\cityTemps.txt', False)
    #hCluster(points, Cluster.singleLinkageDist, 10, False)
    points = buildCityPoints('C:\\Users\\Nate\\Documents\\edx-6.00.2x-problem-sets\\problem_set_6\\cityTemps.txt', True)
